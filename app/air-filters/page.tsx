...
export default function AirFiltersPage() {
  ...
  const handleSidebarFilterChange = (filters: any) => {
    ...
    // OWASP: Validate and sanitize MERV rating filter
    if (filters.mervRatings && Array.isArray(filters.mervRatings) && filters.mervRatings.length > 0) {
      const validMervRatings = filters.mervRatings.filter((r: any) => 
        Number.isInteger(r) && r >= 1 && r <= 20
      );
      if (validMervRatings.length > 0) {
        filtered = filtered.filter(product => {
          if (!('mervRating' in product)) return false;
          const pRating = typeof product.mervRating === 'string' ? parseInt(product.mervRating, 10) : product.mervRating;
          if (isNaN(pRating) || !Number.isInteger(pRating)) return false;
          return validMervRatings.includes(pRating);
        });
      }
    }
  ...
  // Calculate price range from displayed products
  const calculatedPriceRange = useMemo(() => {
    if (displayedProducts.length === 0) return [0, 200];
    const prices = displayedProducts.map(p => p.price || 0).filter(p => p > 0);
