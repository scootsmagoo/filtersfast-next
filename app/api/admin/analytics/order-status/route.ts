...
import { getOrdersByStatus } from '@/lib/db/analytics';
...
export async function GET(request: NextRequest) {
  ...
  if (!checkAnalyticsRateLimit(identifier, 'analytics:order-status')) {
    return NextResponse.json(
  ...
  try {
    ...
    const ordersByStatus = getOrdersByStatus(startDate, endDate);
