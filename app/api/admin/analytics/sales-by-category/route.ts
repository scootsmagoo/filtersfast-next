...
import { getSalesByCategory } from '@/lib/db/analytics';
...
export async function GET(request: NextRequest) {
  ...
  if (!checkAnalyticsRateLimit(identifier, 'analytics:sales-by-category')) {
    return NextResponse.json(
  ...
  try {
    ...
    const salesByCategory = getSalesByCategory(startDate, endDate);

    await auditLog({
      action: 'analytics_access',
      userId: session.user.id,
      resource: 'analytics',
      resourceId: 'sales-by-category',
      status: 'success',
