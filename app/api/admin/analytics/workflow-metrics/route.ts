...
import { getWorkflowMetrics } from '@/lib/db/analytics';
...
export async function GET(request: NextRequest) {
  ...
  if (!checkAnalyticsRateLimit(identifier, 'analytics:workflow-metrics')) {
    return NextResponse.json(
  ...
  try {
    ...
    const metrics = getWorkflowMetrics(startDate, endDate);

    await auditLog({
      action: 'analytics_access',
      userId: session.user.id,
      resource: 'analytics',
      resourceId: 'workflow-metrics',
      status: 'success',
