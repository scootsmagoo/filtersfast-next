...
import { getWishlistMetrics } from '@/lib/db/analytics';
...
export async function GET(request: NextRequest) {
  ...
  if (!checkAnalyticsRateLimit(identifier, 'analytics:wishlist-metrics')) {
    return NextResponse.json(
  ...
  try {
    ...
    const metrics = getWishlistMetrics(startDate, endDate);

    await auditLog({
      action: 'analytics_access',
      userId: session.user.id,
      resource: 'analytics',
      resourceId: 'wishlist-metrics',
      status: 'success',
    ...
    console.error('Error fetching wishlist metrics:', error);
    return NextResponse.json(
      { error: 'Failed to fetch wishlist metrics' },
      { status: 500 }
    );
  }
}
