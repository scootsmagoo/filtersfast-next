...
import { getCustomerLTVMetrics } from '@/lib/db/analytics';
...
export async function GET(request: NextRequest) {
  ...
  if (!checkAnalyticsRateLimit(identifier, 'analytics:customer-ltv')) {
    return NextResponse.json(
  ...
  try {
    ...
    const ltvMetrics = getCustomerLTVMetrics(startDate, endDate);

    await auditLog({
      action: 'analytics_access',
      userId: session.user.id,
      resource: 'analytics',
      resourceId: 'customer-ltv',
      status: 'success',
    ...
    console.error('Error fetching customer LTV metrics:', error);
    return NextResponse.json(
      { error: 'Failed to fetch customer LTV metrics' },
      { status: 500 }
    );
  }
}
