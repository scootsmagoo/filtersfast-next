...
function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    ...
    case 'LOAD_STATE': {
      // Safety check: ensure payload has required structure
      const safePayload = action.payload || {};
      const items = Array.isArray(safePayload.items) ? safePayload.items : [];
      const appliedGiftCards = Array.isArray(safePayload.appliedGiftCards) ? safePayload.appliedGiftCards : [];
      const appliedDeals = Array.isArray(safePayload.appliedDeals) ? safePayload.appliedDeals : [];
    ...
    if (savedCart) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          // Safety checks: ensure all fields are arrays
